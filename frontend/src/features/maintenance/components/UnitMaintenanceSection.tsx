...
interface UnitMaintenanceSectionProps {
  propertyId: string;
  unitId: string;
}

/**
 * Unit-scoped zoom level of the Maintenance feature — same table and
 * drawer as the global /maintenance page and PropertyMaintenanceSection,
 * fixed to this one unit. propertyId is still needed: a new work order
 * is always created under a property (see WorkOrderHandler.Create), the
 * unit is just this section's filter on top of that.
 */
export function UnitMaintenanceSection({ propertyId, unitId }: UnitMaintenanceSectionProps) {
